...
function extend<T extends typeof THREE.MeshStandardMaterial>(
    source: T,
    object
) {
    object = object || {};
...
function useUniforms(name, object, uniforms) {
    ...
    const fog =
        object.fog ||
        (object.material
            ? object.material.fog || object.material.useFog
            : null);
    const lights =
    ...
    if (lights) {
        if (object.use) {
            ...
            if (use.indexOf("SpotLight") > -1) {
                cloneUniforms(
                    UniformsLibExtendable.lights.spotLights,
                    uniforms
                );
                ...
                if (shadows) {
                    ...
                    cloneUniforms(
                        UniformsLibExtendable.spotShadowMap,
                        uniforms
                    );
                    cloneUniforms(
            ...
            if (use.indexOf("LightProbe") > -1)
                cloneUniforms(
                    UniformsLibExtendable.lights.lightProbe,
                    uniforms
                );
